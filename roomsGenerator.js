...
class Role {
    ...
    addSkill(skill) {
        if (skill instanceof Skill) {
            ...
            throw `Expected Skill(name: string, value: number) type for adding a skill to role, instead got [constructor: ${skill.constructor}, type: ${typeof skill}, value: ${skill}]`;
        }
    }
...
class Gear {
    ...
    set armor(x) {
        if (x instanceof Armor) {
            ...
            throw `Expected Armor() type for gear armor, instead got [constructor: ${x.constructor}, type: ${typeof x}, value: ${x}]`;
        }
    }
    get weapons() {
    ...
    set weapons(x) {
        if (x instanceof Array) {
            ...
            throw `Expected Array type for gear weapons, instead got [constructor: ${x.constructor}, type: ${typeof x}, value: ${x}]`;
        }
    }
    addWeapon(weapon) {
        if (weapon instanceof Weapon) {
            ...
            throw `Expected Weapon() type for gear weapons, instead got [constructor: ${weapon.constructor}, type: ${typeof x}, value: ${x}]`;
        }
    }
    get cyberware() {
    ...
    set cyberware(x) {
        if (x instanceof Array) {
            ...
            throw `Expected Array type for gear cyberware, instead got [constructor: ${x.constructor}, type: ${typeof x}, value: ${x}]`;
        }
    }
    addCyberware(x) {
        if (x instanceof Cyberware) {
            ...
            throw `Expected Cyberware() type for gear cyberware, instead got [constructor: ${x.constructor}, type: ${typeof x}, value: ${x}]`;
        }
    }
    addToGear(x) {
        let handleIndividual = (gear) => {
            if (x instanceof Weapon) {
                ...
                throw `Expected Weapon() or Cyberware() type for adding to gear, instead got [constructor: ${x.constructor}, type: ${typeof x}, value: ${x}]`;
            }
        };
        if (x instanceof Array) {
            x.forEach(gear => {
            ...
            throw `Expected Array of Weapon() or Cyberware(), or single Weapon() or single Cyberware() type for adding to gear, instead got [constructor: ${x.constructor}, type: ${typeof x}, value: ${x}]`;
        }
    }
}
class Cyberware {
...
const weapons = {
    ...
    lightPistol: [
        ...
        new Weapon('Federated Arms X-22', 'Handgun', 0, '1d6+1', 10, 2, Reliability.ST, 50),
    ],
    mediumPistol: [
    ...
    lightAssaultRifle: [new Weapon('Militich Ronin Light Assault', 'Rifle', 1, '5d6', '5.56', 35, 30, Reliability.VR, 400)],
...
class Enemy {
    constructor(name, difficulty) {
        ...
        this.gear = this.generateGear(difficulty);
        this.stirUpSkills(this.role.skills);
        this.generatePickupSkills();
    };
    get name() {
        return this._name;
    };
    ...
    set role(x) {
        if (x instanceof Role) {
            ...
            throw `Expected Role() type for enemy role, instead got [constructor: ${x.constructor}, type: ${typeof x}, value: ${x}]`;
        }
    }
    get stats() {
        return this._stats;
        ...
        if (x instanceof Array) {
            x.forEach(stat => {
                ...
                if (stat instanceof Stat) {
                    ...
                    throw `Expected Stat() type for enemy stats, instead got [constructor: ${stat.constructor}, type: ${typeof x}, value: ${x}]`;
                }
            });
        } else {
    ...
    set gear(x) {
        if (x instanceof Gear) {
            ...
            throw `Expected Gear() type for enemy stats, instead got [constructor: ${x.constructor}, type: ${typeof x}, value: ${x}]`;
        }
    }
    ...
    generateGear(difficulty) {
        let weapon = null;
        let armor = null;
        let roleModifier = 0;
        if (this.role.name === 'nomad') {
            roleModifier = 2;
        } else if (this.role.name === 'solo') {
            roleModifier = 3;
        }
        ...
        let gear = new Gear(armor, weapon);
        this.generateCyberware(role.name);
        return gear;
    };
    generateCyberware(gear) {
        if (gear instanceof Gear) {
            let iterations = 3;
            if (this.role.name === Roles[2]) {
                iterations += 3;
            }
            let rolls = [];
                ...
                switch (roll) {
                    ...
                    case 1: // cyberarm
                        if (rolls.indexOf(1) > -1) {
                            ...
                            switch (cyberarm) {
                                case 0:
                                    gear.addToGear(
                                        weapons.mediumPistol[Math.floor(Math.random() * weapons.mediumPistol.length)]
                                    );
                                    break;
                                case 1:
                                    gear.addToGear(
                                        weapons.lightPistol[Math.floor(Math.random() * weapons.lightPistol.length)]
                                    );
                                    break;
                                case 2:
                                    gear.addToGear(
                                ...
                                case 3:
                                    gear.addToGear(
                                        weapons.lightSMG[Math.floor(Math.random() * weapons.lightSMG.length)]
                                    );
                                    break;
                                case 4:
                                    gear.addToGear(
                                        weapons.veryHeavyPistol[Math.floor(Math.random() * weapons.veryHeavyPistol.length)]
                                    );
                                    break;
                                case 5:
                                    gear.addToGear(
                                        weapons.heavyPistol[Math.floor(Math.random() * weapons.heavyPistol.length)]
                                    );
                                    break;
                            }
                        }
                    ...
                    case 2: // cyberaudio
                        if (rolls.indexOf(2) > -1) {
                            ...
                            rolls.push(2);
                            gear.addToGear(
                                cyberwares.cyberaudio[Math.floor(Math.random() * cyberwares.cyberaudio.length)]
                            );
                        }
                        break;
                    case 3:
            ...
            throw `Expected Gear() type for cyberware generator, instead got: [constructor: ${gear.constructor}, type: ${typeof gear}, value: ${gear}]`;
        }
    };
    stirUpSkills(skills) {
        for (let i = 0; i < 50; ++i) {
    ...
    generatePickupSkills() {

    }
    addSkill(skill) {

    }
    // here begins the skill shit
    stirUpSkills(self.role.skills);
    let pickupPoints = self.stats.ref + self.stats.int;
// first, let's check if they even have skills for the weapon they're using.
let newSkills = [];
self.gearPack.weapon.forEach(weapon => {
    self.role.skills.forEach(skill => {
...
if (self.role.name !== 'nomad') {
    movement.push(new Skill('Endurance', 0));
}
if (self.role.name !== 'solo' && self.role.name !== 'nomad') {
...
stirUpSkills(newSkills);
// k.  now let's attach 'em to the main list
self.role.skills.concat(newSkills);
}
...
function Room(name, enemyArray) {
    ...
    enemyArray.forEach((enemyType, difficulty) => {
        ...
        for (let i = 0; i < enemyType; ++i) {
            ...
            let statNums = rollStats();
            let gearPack = getGearPack(difficulty, role);
            enemies.push(new Enemy(name, role, statNums, gearPack));
        }
        self.enemies.push(enemies);
    });
}
